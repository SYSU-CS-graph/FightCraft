...
func _ready() -> void:
	...
	generate_sine_wave_terrain(-29,-40,35,10)
	# 生成水
	water_init()
	#river_init()
	# 随机生成点东西
...
func layer_init():									# 初始化平台
	...
	for i in range(101):
		for j in range(101):
			if pool_star<=i and i<=pool_end and pool_star<=j and j<=pool_end:
				continue
...
func generate_sine_wave_terrain(x_pos : int, z_pos: int , range:int , max_height:int):
	...
	for x_temp in range(terrain_size):
		for z_temp in range(terrain_size):
			...
			for y in range(height):
				var block = preload("res://资源场景/方块/泥土方块.tscn").instantiate()
				add_child(block)
...
func water_init():									# 生成池塘
	...
	for i in range(30, 51):
		for j in range(30, 51):
			# 竖直边缘
			if i == 30 or i == 50 or j == 30 or j == 50:
