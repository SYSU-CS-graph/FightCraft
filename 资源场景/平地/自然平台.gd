	...
	for i in range(101):
		for j in range(101):
			if 40<=i and i<=46 and 40<=j and j<=46:
				continue
			# 创建地基方块
			var each_node = preload("res://资源场景/方块/方块_new_base.tscn").instantiate()
			# 添加到当前场景中
			add_child(each_node)
			# 设定该方块的位置
		...
		for j in range(40, 47):
			...
			if i == 40 or i == 46 or j == 40 or j == 46:
				for k in range(depth): # y坐标
					var each_node_1 = preload("res://资源场景/方块/方块_new_base.tscn").instantiate()
					add_child(each_node_1)
					each_node_1.position = base_position + Vector3(i - 50, -k, j - 50)
					hash_node_pool[Vector3(i - 50, -k, j - 50)] = each_node_1
			# 池底
			var each_node_2 = preload("res://资源场景/方块/方块_new_base.tscn").instantiate()
			add_child(each_node_2)
			each_node_2.position = base_position + Vector3(i - 50, -depth, j - 50)
			hash_node_pool[Vector3(i - 50, -depth, j - 50)] = each_node_2
